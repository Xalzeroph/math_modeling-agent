...
\subsection{问题四的分析}
问题四从匀速模型过渡到真实运动学。堆垛机受加速度限制，无法瞬间达到最大速度——水平加速需6s行驶9m才到3.0m/s，短距行程全程未达最大速度便进入减速。需量化线性模型在仓库典型行程下的低估量，并分析非线性是否改变P1的货位排序——若排序不变，P1方案对速度模型鲁棒，无需重算。
...
表\ref{tab:p1_result}显示，贪心启发式分配65000箱全部满足层高约束，每种材料分配数量与库存数量完全一致。GA仅带来0.00\%的额外改善，说明热度排序策略已接近该问题的最优解---在高维组合优化空间中，构造式贪心算法对按频率-距离乘积排序的分配问题具有天然的近似最优性。
...
在问题一确定最优货位方案的基础上，本节基于该分配方案和一周订料数据，研究纯出库场景下7台堆垛机的作业调度。
...
\begin{equation}
T_{\text{DC}}=\max\left(\frac{d_{h,\text{out}}}{v_h^{\text{empty}}},\frac{d_{v,\text{out}}}{v_v^{\text{empty}}}\right)+T_{\text{load}}+\max\left(\frac{|\Delta d_h|}{v_h^{\text{empty}}},\frac{|\Delta d_v|}{v_v^{\text{empty}}}\right)+T_{\text{load}}+\max\left(\frac{d_{h,\text{in}}}{v_h^{\text{empty}}},\frac{d_{v,\text{in}}}{v_v^{\text{empty}}}\right)
\end{equation}

其中各段均使用空载速度$v_h^{\text{empty}},v_v^{\text{empty}}$（堆垛机取放货后不需要满载速度移动），DC的三段使用单程时间函数$\max(d/v)$，不可复用SC的往返函数$2\times\max(d/v)$。DC相对于两次SC的节省量为：
\begin{equation}
...
梯队内选最短行程：
\begin{equation}
s^*=\arg\min_{s\in\text{梯队}(i)\cap\cS_{\text{type}(i)}}t_{\text{in}}(s)
\end{equation}
...
线性速度模型在仓库典型行程下系统性低估50\%-500\%。宏观分配用线性、精细调度用非线性，两种模型下货位排序一致，分配方案对速度模型鲁棒。
