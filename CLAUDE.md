...
| 21 | `notes/formula_code_map.md` | S5 | S7, S11 | S7 审计抽检时缺失 → BLOCKED; S11 承诺-实现对照缺失 → WARN |
| 22 | `eval_report.json` | S13 | S14 | S14 FAIL |
| 23 | `提交.zip` | S15 | — | 提交物缺失 |
...
| 14 | 跳过 §0.1 规则 4/5/6（通读全文 / 学习证据 / 认知自检）直接执行 |
| 15 | 用户说"继续" ≠ 可以跳过学习步骤。进入新 Stage 必须从头运行入口自检 + 学习步骤
| 16 | 角色A公式未实现即声称"模型已完成" — 每个标注为"角色A"的编号公式必须在 S5 运行前有对应 solver 代码和数值输出 |
| 17 | 审计时跳过 formula_code_map.md 的抽样验证 — S7 审稿手必须抽检至少 3 个角色A公式的代码实现 |
...
3. **✱ 运行前自检**（代码写完→立即做→通过后才准运行）:
   - **(a) 公式-代码逐行对照**: 把 ProblemAnalysis.md 中该子问题的所有编号公式抄到 `notes/formula_code_map.md`（格式见 `roles/编程手.md §9.3`），每个公式旁写下它在代码中的行号。区分角色A（须实现）和角色B（论文推导，不需实现）。找不到对应行且标注为角色A的公式 → 代码缺了，补完再继续。
   - **(b) 参数值核对**: 代码中的每个物理参数/算法参数与模型文档声明的值一一比对。不一致 → 修正代码。
   - **(c) 逻辑走向检查**: 算法的"主循环→分支→输出"路径是否与模型设计的流程图一致。有 `break`/`continue`/`return` 的地方逐行注释说明其目的。
   - **(d) 公式实现完整性检查**: 扫描 formula_code_map.md，所有角色A公式的实现状态不得为 `❌ 未实现` 或 `⚠️ 简化实现`（除非建模手书面确认）。存在任意未实现的角色A公式 → BLOCKED，不准运行。
...
| # | 致命度 | 检查 | 通过标准 |
|---|--------|------|---------|
| 1 | 🔴 | **代码-模型一致性** ★ | 读 `notes/formula_code_map.md`（编程手 S5 产出），从中抽检 3-5 个角色A公式，逐个到 solvers/ 代码中找到对应实现行，验证输入输出值一致。公式在代码中缺失、或逻辑被改写为与公式不同 → FAIL。mapping 不存在 → BLOCKED（退回 S5 补）。抽检全部 PASS → 其余代码推定一致。
...
| 18 | **禁止全文中出现任何括号注释** | 正文、表格、假设、说明等所有部分均不得出现中文或英文括号注释。必要的缩写直接书写不加括号，数值范围用"至"或"—"连接。唯一允许括号的地方是数学公式模式和LaTeX命令参数。|
| 19 | **摘要加粗三类全覆盖** 
| 20 | **聚焦核心创新，不铺陈无关对比** | 论文的核心价值在模型创新、求解方法和结果分析。禁止：用多个策略/算法罗列凑篇幅、展示无差异的对比、把"试过的方案"当"贡献"写进去。每节自问：这部分是问题的核心解决路径吗？不是→删。|
| 21 | **对比聚焦核心方法** | 论文中对比是加分项，但只对比核心方法——GA vs 贪心启发式 √、SC vs DC √、线性 vs 非线性 √。不对比微调变体——FIFO vs NN vs FP vs HYB ×、DP聚簇 vs 独立处理 ×。判断标准：这个对比能改变读者对问题的理解吗？不能→不写。|
| 22 | **建模期选型 vs 论文期呈现** | 建模阶段可以尝试多种方案以选最优，这是工作过程。论文只呈现有意义的对比，不把"试过的"都写进去。对比的目的是展示不同方法论的优势，不是记录探索过程。| 
